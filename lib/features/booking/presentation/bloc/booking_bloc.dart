...
class BookingBloc extends Bloc<BookingEvent, BookingState> {
	...
	BookingBloc({
		...
		Future<void> _onBookAppointment(BookAppointmentRequested event, Emitter<BookingState> emit) async {
			emit(BookingLoading());
			try {
	...
	Future<void> _onLoadAppointments(LoadAppointmentsRequested event, Emitter<BookingState> emit) async {
		emit(BookingLoading());
		try {
	...
	Future<void> _onUpdateStatus(UpdateAppointmentStatusRequested event, Emitter<BookingState> emit) async {
		emit(BookingLoading());
		try {
			await bookingRepository.updateAppointmentStatus(event.appointmentId, event.status);
